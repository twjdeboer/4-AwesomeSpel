...
public class ProceduralHouse{
	...
	public void MakeDoor(){
		...
		temp.z += (float)Random.Range ((float)-0.45 * scale.z, (float)0.45 * scale.z);
		doorypos = temp.y;
		doorzpos = temp.z;
	...
	private bool windowsetter(int i){
		...
		//MonoBehaviour.print (temp.y+" temp.y ");
		
		//MonoBehaviour.print ((a == 1)+" "+a+" a "+i);
		if (a == 1) {
	...
	private void randomDim(){
		int n = 1;
		float x = (float)scale.x + (float)Random.Range (-n,n*3);
